...
class Main {
    public static void main(String[] args) throws AWTException, IOException, InterruptedException {
        ...
        createOutputDir(ss.outputDir);

        for(int k = 0 ; k < ss.numOfScreenshots; ++k) {
            BufferedImage image = robot.createScreenCapture(screen);
            ImageIO.write(image, "png", new File(ss.outputDir + "screenshot" + k + ".png"));
            System.out.println("Captured image : screenshot" + k);
            robot.delay((int) ss.interval*1000);
        }
